...
export default function GenreSelectionDialog() {
  ...
  const [showDialog, setShowDialog] = useState(true); // <- kontrol render setelah animasi exit
  ...
  const handleContinue = () => {
    if (selectedGenres.length > 0) {
      ...
      setIsOpen(false); // trigger animasi keluar
      setTimeout(() => setShowDialog(false), 400); // tunggu animasi exit selesai sebelum unmount
    }
  };
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            transition={{ duration: 0.4, ease: 'easeInOut' }}
          >
            <div className="p-7 max-w-2xl max-h-[100vh] rounded-lg shadow-lg text-center flex flex-col lg:gap-5 md:gap-5 sm:gap-4 gap-4 
              ...
              {/* Grid Genre (Scrollable + Custom Scrollbar) */}
              <div
